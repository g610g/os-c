...
Process* initialize_process(int* length){
    ...
    scanf("%d", length);
    Process* processes = malloc(sizeof(Process) * (MAX_LEN));
...
void calculate(Process* processes, int length, int slice_time){
    printf("Output\n");
    ...
    for(int i = 0; i < length; i++){
        if (processes[i].burst_time > slice_time){
            processes[i].waiting_time = total_waiting;
            processes[i].burst_time -= slice_time;
            processes[i].turnaround_time = processes[i].waiting_time + slice_time;
            printf("P:%d\t\t %d\t\t\t\t%d\t\t\t\t%d\t\t\t\t%d\n", processes[i].id,processes[i].priority_id, slice_time, processes[i].waiting_time, processes[i].turnaround_time);
            total_waiting = slice_time + total_waiting;
            total_turnaround += processes[i].turnaround_time;
            processes[length] = processes[i];
            length++;
        }else if (processes[i].burst_time <= slice_time){
            processes[i].waiting_time = total_waiting;
...
int main(void){
    ...
    scanf("%d", &slice_time);
    calculate(processes, length, slice_time);
    free(processes);
}